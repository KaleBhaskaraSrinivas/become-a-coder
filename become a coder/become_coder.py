...
'''
HIGHEST ELEMENT IN STRING POSITION AND LENGTH
n=input()
data=n.split()
m=len(data[0])
val=data[0]
...
'''
PRINTING A STRING WORD IN REVERSE ORDER
...
'''
PRINTING COUNT OF A STRING OF EACH WORD IN THAT LENGTH
...
'''
PRINTING ORDER OF STRING IN REVERE
...
'''
'''
COUNT OF WORDS WITHOUT SPLIT IN A STRING
...
'''
compare between two string same word means print words and count in a strings

st1=input().split()
st2=input().split()

def word_compare(st1,st2):
    newstr=[]
    for i in st1:
        if i in st2 and i not in newstr:
            newstr.append(i)
    return len(newstr), " " . join(newstr)
count,string=word_compare(st1,st2)
print(count)
print(string)
'''

'''
COMPARING TWO STRINGS AND PRINTING COMMON LETTER IN STRINGS
